...
public abstract class Unit implements Movable, Serializable {
    /** abstract - ключевое слово, implements - реализация интерфейса
     * Serializable - сериализация, перевод обьекта в поток байтов, который можно записать на дискету
     * Deserializable - "считываем игровой мир с диска" - считывает обьект с диска и превращает его в объект в коде
     */
    private int force;
    private int health;
    private int energy;
    ...
    public void attack(Unit target) { //Атака базовая, атака дальнобойная
        if (target == this) {
            return;
        }
